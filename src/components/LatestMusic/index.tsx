      ...
      <ul>
        {
          dataList.map((item: any, index: number) => {
            return (
              <li className={styles.item} key={item.id} onDoubleClick={() => Song.getSongUrl(item.id)}>
                ...
                <p className={styles.content}>
                  ...
                  {
                    item.transNames && item.transNames.map((items: any, index: number) => {
                      return <i className={styles.smallTip} key={index}>{items}</i>
                    })
                  }
                  {
                    item.transNames && <span>)</span>
                ...
                <p className={styles.name}>
                  {
                    item.artists && item.artists.map((nameItem: any, index: number) => {
                      return (
                        <i key={nameItem.id}>
                          {nameItem.name}
                ...
                <p className={styles.title}>
                  ...
                  {
                    item.album.transNames && item.album.transNames.map((albumItem: any, index: number) => {
