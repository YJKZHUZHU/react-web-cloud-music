...
const Login = () => {
  ...
  const onFinish = async (values: IFormData) => {
    try {
      ...
      if (passwordType === PasswordTypeEnum.PHONE) {
        ...
        toggle(false)
        if (LoginRet.code !== 200) return message.info("密码错误")
        LoginRet.setLoginCache(false, true, LoginRet.profile.userId, LoginRet.cookie)
      }
  ...
  const renderFormItem = () => {
    ...
    if (loginType === LoginTypeEnum.PASSWORD) {
      return (
        <>
          {passwordType === PasswordTypeEnum.EMAIL ? (
            <Form.Item
              name="email"
              required={false}
              rules={[
      ...
      <div className="flex w-[100%] h-[100%]">
        <div className={classNames(styles.formLayout)}>
          <div className={classNames(styles.form)}>
            {qrLogin ? (
              <QrLogin callback={onQrCallback} />
            ) : (
              <div className={classNames("w-[320px]", "flex", "flex-col")}>
                ...
                <Tabs
                  ...
                  centered
                  activeKey={loginType}
                  onChange={(activeKey) => setLoginType(activeKey as LoginTypeEnum)}></Tabs>
                <Form {...LAYOUT} onFinish={onFinish} initialValues={INIT_FORM} form={form}>
                  {renderFormItem()}
                    ...
                    {loginType === LoginTypeEnum.PASSWORD && (
                      <span
                        ...
                        onClick={() =>
                          setPasswordType(
                            passwordType === PasswordTypeEnum.EMAIL
                              ? PasswordTypeEnum.PHONE
                              : PasswordTypeEnum.EMAIL
                          )
                        }>
                        {passwordType === PasswordTypeEnum.EMAIL ? "手机号登录" : "邮箱登录"}
                      </span>
                    )}
            ...
            <div
              ...
              className={classNames(
                styles.bgImage,
                qrLogin ? styles.desktop : styles.qrCode
              )}></div>
            <div onClick={onVisitor} className={classNames(styles.visitor)}>
              {visitorLoading && <Spin size="small" />}游客登录
            </div>
          </div>
        </div>
      </div>
